...
bool isValidPair(int x, int y) {
    ...
    const ivec2 validPairs[4] = ivec2[](
        ivec2(1, 1), ivec2(1, 3),ivec2(3, 1), ivec2(3, 3)
    );
...
void main() {
    ...
    if(isValidPair(lid.x, lid.y)){
        vec4 a = sharedBlock[lid.x-1][lid.y-1] ;
        vec4 b = sharedBlock[lid.x+1][lid.y-1] ;
        vec4 c = sharedBlock[lid.x+1][lid.y+1] ;
        vec4 d = sharedBlock[lid.x-1][lid.y+1] ;
