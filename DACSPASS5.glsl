...
bool isValidPair(int x, int y) {
    ...
    const ivec2 validPairs[8] = ivec2[](
        ivec2(0, 1), ivec2(0, 3),ivec2(1, 0), ivec2(1, 2),
        ivec2(2, 1), ivec2(2, 3),ivec2(3, 0), ivec2(3, 2)
    );

    for (int i = 0; i < validPairs.length(); i++) {
        if (((validPairs[i].x == x) || (validPairs[i].x +4 == x)) && ((validPairs[i].y == y) || (validPairs[i].y+4 == y))) {
            return true;
        }
    }
    return false;
...
void main() {
    ...
    sharedBlock[lid.x+1][lid.y+1] = imageLoad(inputImage, gid);

    if(lid.x == 0 ){
        sharedBlock[0][lid.y] = imageLoad(inputImage, gid - ivec2(0,1));
    }
    if(lid.y == 0 ){
        sharedBlock[lid.x][0] = imageLoad(inputImage, gid - ivec2(1,0));
    }
    ...
    if(isValidPair(lid.x, lid.y)){
        vec4 a = sharedBlock[lid.x+1][lid.y] ;
        vec4 b = sharedBlock[lid.x][lid.y+1] ;
        vec4 c = sharedBlock[lid.x+1][lid.y+2] ;
        vec4 d = sharedBlock[lid.x+2][lid.y+1] ;
